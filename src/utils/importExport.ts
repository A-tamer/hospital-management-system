...
/**
 * Generate patient code in format: YYYYMMXXXX (no slashes)
 * Example: 2024 11 0001 -> 2024110001
 */
export function generatePatientCode(patients: Patient[] = []): string {
  ...
  const serialNumbers = currentMonthPatients
    .map(p => {
      ...
      // Support old format: YYYY/MM/XXXX
      if (code.includes('/')) {
      ...
      // New format: YYYYMMXXXX
      if (code.length === 10 && code.startsWith(year.toString() + month)) {
        const serialPart = code.slice(6); // last 4 characters
  ...
  // New code format without slashes
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          // Generate code for this patient (YYYYMMXXXX)
